  ...
  <motion.footer variants={footerVariants} initial="hidden" whileInView="show" className={`${styles.paddings} py-8 relative`}>
    ...
    <div className={`${styles.innerWidth} flex flex-col gap-8 mx-auto`}>
      <div className="flex items-center justify-between flex-wrap gap-5">
        ...
        <button className="flex items-center py-4 px-6 bg-[#25618b] h-fit rounded-[32px] gap-[12px]">
      ...
      <div className="flex flex-col">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex gap-4">
            {socials.map((social) => (
              <img src={social.url} key={social.name} alt="" className='w-[24px] h-24px object-contain cursor-pointer' />
            ))}
          </div>
        </div>
      </div>
